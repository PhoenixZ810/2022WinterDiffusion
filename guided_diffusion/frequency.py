...
# print (img_path)
img_path = "/home/zxy/2023WinterDiffusion/data_dir/FDST/test_data_small/5/085.jpg"
# img_path = "/home/zxy/2023WinterDiffusion/data_dir/FDST/train_data/81/079.jpg"
# img_path = "/home/zxy/2023WinterDiffusion/data_dir/FDST/test_data1_small/5/085.jpg"
...
h_res, w_res = 160, 256
img = cv2.resize(img, (w_res, h_res), interpolation=cv2.INTER_AREA)[:, :, ::-1]

'''对RGB图像进行fft变换'''
# plt.figure(figsize=(6.4 * 5, 4.8 * 5), constrained_layout=False)
# img_c1 = cv2.imread(img_path, 0)
img_c1 = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
...
'''边缘检测'''
canny1 = cv2.Canny(img_c1, 50, 150).astype(float)
# print(canny1)
canny1[np.where(canny1 == 255)] = 1
print(canny1)
...
# plt.show()
plt.imshow(canny1, 'gray'), plt.title("Edge of Original Image")
plt.show()
...
'''resize scale k'''
for i in range(0, len(anno_list)):
    y_anno = min(int(float(anno_list[i]['shape_attributes']['y']) / rate_h) + 3, 159)
    x_anno = min(int(float(anno_list[i]['shape_attributes']['x']) / rate_w) + 3, 255)
    # y_anno = min(int(anno_list[i]['shape_attributes']['y'] / rate_h), 159)
    # x_anno = min(int(anno_list[i]['shape_attributes']['x'] / rate_w), 255)
    # y_anno = anno_list[i]['shape_attributes']['y']
    # x_anno = anno_list[i]['shape_attributes']['x']
    k[y_anno, x_anno] = 1
# # pdb.set_trace()
k = gaussian_filter(k, 2)*100
print(k)
# pdb.set_trace()
plt.imshow(k, cmap=CM.jet), plt.title("den")
plt.show()
fusion = k + canny1
plt.imshow(fusion, cmap=CM.jet), plt.title("edge+den")
plt.show()
